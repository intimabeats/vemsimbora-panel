import React, { useState, useEffect } from 'react'
    ...
    import { actionTemplateService } from '../../services/ActionTemplateService'; // Import ActionTemplateService
    ...
    export const CreateTaskModal: React.FC<CreateTaskModalProps> = ({
      ...
      }, [isOpen, formData.difficultyLevel])
      ...
      const handleActionChange = (index: number, field: keyof TaskAction, value: any) => {
        setFormData(prev => {
          ...
          const updatedAction: TaskAction = { ...newActions[index], [field]: value };
      ...
      const handleAddActionFromTemplate = async () => { // Make async
        if (!selectedTemplate) return;

        try {
          const fullTemplate = await actionTemplateService.getActionTemplateById(selectedTemplate); // Await the promise
          if (!fullTemplate) return;

          // Deep copy the elements and assign new IDs
          const newActions: TaskAction[] = JSON.parse(JSON.stringify(fullTemplate.elements)).map((action: TaskAction) => ({
            ...action,
            id: Date.now().toString() + Math.random().toString(36).substring(7), // Generate unique ID
          ...
          setError("Failed to add action from template."); // Set an error message
        }
      };
