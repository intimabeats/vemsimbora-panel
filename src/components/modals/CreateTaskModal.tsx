import React, { useState, useEffect } from 'react'
  ...
  export const CreateTaskModal: React.FC<CreateTaskModalProps> = ({
    ...
    const [formData, setFormData] = useState({
      ...
      assignedTo: [] as string[], // Keep assignedTo
      priority: 'medium' as TaskSchema['priority'], //Keep priority
      dueDate: new Date().toISOString().split('T')[0], //Keep due date
      difficultyLevel: 5,
      coinsReward: 50
    });
      ...
      useEffect(() => {
        const fetchData = async () => {
          try {
            ...
            setError('Erro ao carregar projetos e usuários') // Keep this error, as it's specific to data fetching
          }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        console.log("Task created:", newTask); // Log newly created task
        onTaskCreated(newTask);
        onClose();
      } catch (err: any) {
        console.error("Error creating task:", err); // Log any errors
